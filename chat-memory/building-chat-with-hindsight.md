...
2. Hindsight.recall() → Get relevant memories
     ↓
3. Groq API → Generate response with memory context
     ↓
...
- Docker for running Hindsight

## Step 1: Setting Up Hindsight

First, let's start the Hindsight API server:
...
This gives us:
- **Hindsight API**: http://localhost:8888 (for our app to use)
- **Control Plane UI**: http://localhost:9999 (for viewing stored memories)
...
```bash
npm install @vectorize-io/hindsight-client groq-sdk marked uuid @tailwindcss/typography
...
## Step 5: Groq Integration for Fast AI Responses
...
export async function POST(request: NextRequest) {
  try {
    ...
    // 3. Generate response using Groq with memory context
...
3. **Context-Aware Generation**: Groq generates responses knowing what the user has shared before
...
Visit http://localhost:9999 to see your stored memories in the Hindsight Control Plane!
...
5. **AI Response**: Groq generates a response with full knowledge of past conversations
...
We've built a chat application that remembers! By integrating Hindsight with Next.js and Groq, we created an AI assistant that:
